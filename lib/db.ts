...
// Data persists during serverless function lifetime (warm starts)
...
const SEED_USERS: DBUser[] = [
  { id: 1, name: "Abebe Kebede", email: "abebe@smu.edu.et", password: "password123", role: "student", is_approved: true, smu_id: "RCD/0045/2020", department: "Computer Science", gpa: 3.75, skills: ["JavaScript", "React", "Node.js", "Python", "SQL"], bio: "Passionate software developer interested in web technologies and AI.", graduation_year: 2024 },
  { id: 2, name: "Sara Hailu", email: "sara@smu.edu.et", password: "password123", role: "student", is_approved: true, smu_id: "RMD/0023/2021", department: "Marketing", gpa: 3.5, skills: ["Digital Marketing", "Social Media", "Content Strategy", "Analytics"], bio: "Creative marketer with a focus on digital campaigns.", graduation_year: 2025 },
  { id: 3, name: "Dawit Tesfaye", email: "dawit@smu.edu.et", password: "password123", role: "student", is_approved: false, smu_id: "RAD/0012/2022", department: "Accounting", gpa: 3.8, skills: ["QuickBooks", "Excel", "Financial Analysis", "Taxation"], bio: "Detail-oriented accounting student with internship experience.", graduation_year: 2026 },
  { id: 4, name: "Hirut Berhane", email: "hirut@techco.com", password: "password123", role: "employer", is_approved: true, company_name: "TechCo Ethiopia", company_sector: "Technology", company_description: "Leading software development company in Addis Ababa.", company_website: "https://techco.et" },
  { id: 5, name: "Yonas Assefa", email: "yonas@cbe.com.et", password: "password123", role: "employer", is_approved: true, company_name: "Commercial Bank of Ethiopia", company_sector: "Banking & Finance", company_description: "The largest bank in Ethiopia with nationwide presence.", company_website: "https://cbe.com.et" },
  { id: 6, name: "Admin User", email: "admin@smu.edu.et", password: "admin123", role: "admin", is_approved: true },
  { id: 7, name: "Mekdes Girma", email: "mekdes@savechildren.org", password: "password123", role: "employer", is_approved: false, company_name: "Save the Children Ethiopia", company_sector: "NGO", company_description: "International NGO focused on children's rights and welfare.", company_website: "https://savethechildren.org" },
];

const SEED_JOBS: DBJob[] = [
  { id: 1, employer_id: 4, title: "Junior Software Developer", description: "We are looking for a passionate junior developer to join our growing team. You will work on exciting projects using modern web technologies, collaborate with senior developers, and contribute to our innovative software solutions.", requirements: ["JavaScript", "React", "Node.js", "Git"], department: "Computer Science", sector: "Technology", job_type: "full_time", salary_min: 15000, salary_max: 25000, location: "Addis Ababa", is_approved: true, is_featured: true, created_at: "2024-01-15", company_name: "TechCo Ethiopia", application_count: 12 },
  { id: 2, employer_id: 4, title: "Software Engineering Intern", description: "Join our internship program and gain hands-on experience in software development.", requirements: ["Python", "SQL", "Problem Solving"], department: "Computer Science", sector: "Technology", job_type: "internship", salary_min: 5000, salary_max: 8000, location: "Addis Ababa", is_approved: true, is_featured: false, created_at: "2024-01-20", company_name: "TechCo Ethiopia", application_count: 28 },
  ...
  { id: 4, employer_id: 5, title: "Finance Intern", description: "Excellent opportunity for accounting and economics students to gain practical experience.", requirements: ["Excel", "Financial Analysis", "Attention to Detail"], department: "Accounting", sector: "Banking & Finance", job_type: "internship", salary_min: 4000, salary_max: 6000, location: "Addis Ababa", is_approved: true, is_featured: false, created_at: "2024-01-22", company_name: "Commercial Bank of Ethiopia", application_count: 42 },
...
export function readDB() {
  return { users: clone(users), jobs: clone(jobs), applications: clone(applications), nextIds: { ...nextIds } };
}
...
export function createUser(data: Omit<DBUser, "id">): DBUser {
  const user: DBUser = { ...data, id: nextIds.user++ };
  users.push(user);
  return clone(user);
}

export function createApplication(data: Omit<DBApplication, "id">): DBApplication {
  const app: DBApplication = { ...data, id: nextIds.application++ };
...
export function getApplicationsByStudent(studentId: number): DBApplication[] {
  return applications.filter((a) => a.student_id === studentId).map(clone);
}

export function getApplicationsByJob(jobId: number): DBApplication[] {
  return applications.filter((a) => a.job_id === jobId).map(clone);
}

export function hasApplied(studentId: number, jobId: number): boolean {
  return applications.some((a) => a.student_id === studentId && a.job_id === jobId);
}

export function getApprovedJobs(): DBJob[] {
  return jobs.filter((j) => j.is_approved).map(clone);
}
...
export function createJob(data: Omit<DBJob, "id">): DBJob {
  const job: DBJob = { ...data, id: nextIds.job++ };
  jobs.push(job);
  return clone(job);
}
